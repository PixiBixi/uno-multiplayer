...
the ground each theme actually paints:
...
- When it is not your turn every card is unplayable, so the whole row lies flat. It is
  still faded, which costs nothing: the row is uniform either way, and the elevation is
  what carries the comparison when there is one to make.
...
`e2e/layout.spec.ts` drives a game to a hand holding both kinds and asserts that a
playable card is never faded, that an unplayable one is faded but never past 0.71, that
the playable ones sit higher, and that only they carry a shadow.
